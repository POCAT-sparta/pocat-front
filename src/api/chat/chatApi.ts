...
export async function getChatMessages(
  ...
  const res = await apiClient.get<ApiResponse<ChatMessagePage>>(
    `/ws/api/v1/chats/${chatId}/messages?page=${page}&size=${size}`
  );
  return res.data;
}
...
export async function markChatAsRead(chatId: number): Promise<void> {
  await apiClient.patch<ApiResponse<void>>(`/ws/api/v1/chats/${chatId}/read`);
}

/** 채팅방 나가기 */
export async function leaveChat(chatId: number): Promise<void> {
  await apiClient.delete<ApiResponse<void>>(`/ws/api/v1/chats/${chatId}`);
}
